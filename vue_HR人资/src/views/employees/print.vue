      ...
      <el-card>
        ...
        <div v-if="type === 'personal'" id="printbox">
          ...
          <table cellspacing="0" width="100%" class="tableList">
            ...
            <tr>
              <th>出生日期</th>
              <td colspan="6">{{ formData.dateOfBirth | formatData }}</td>
            </tr>
            <tr>
              <th>最高学历</th>
...
export default {
  ...
  methods: {
    async getPersonalDetail () {
      const { data: userInfo } = await getDetailInfo(this.userId) // 获取个人基本信息(顶部)
      const { data: detail } = await reqGetPersonalDetail(this.userId) // 获取个人基本信息(底部)
      this.formData = { ...detail, ...userInfo }
    },
    async getJobDetail () {
      const { data: userInfo } = await getDetailInfo(this.userId)
